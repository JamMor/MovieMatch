...
var ImgPath = "//image.tmdb.org/t/p/";
...
//	source:		  "/cgi-bin/TMDB1.pl",  
        source: function(request, response) {
            $.getJSON("//api.themoviedb.org/3/search/movie?" + key , { query: request.term }, function(movies){
...
//		 html= "<div class='outer'><div class='inner'><span class='left'>" + movie.item.poster_path + "<\/span><span class='right'><strong>&nbsp; &nbsp; " + movie.item.title + "<\/strong>&nbsp; &nbsp; (" + movie.item.release_date +")<\/span></div></div>";
    ...
    //lets show the information that's been selected
        ...
        $('#response').html(html);
        var link = "//api.themoviedb.org/3/movie/" +  thisMovie.id;
        var TMDB_id;
//  alert(link);   
            $.ajax({
            type: 'GET',
...
//                console.dir(json);
            ...
            while(i < total){                       // loop through cast members
                ...
                html.push('<div class="text"> <a href="//api.themoviedb.org/3/person/' + json.casts.cast[i].id + '" class="actor">' + json.casts.cast[i].name, '</a><br />', character,'</div>');
                html.push('</li>');
                i = i+1;   
            }
            ...
            var html = [];
            while(i < total){ 
                html.push('<li <a href="//api.themoviedb.org/3/person/' + json.casts.crew[i].id + '" class="actor" >');
                ...
                html.push('<div class="text"> <a href="//api.themoviedb.org/3/person/' + json.casts.crew[i].id + '" class="actor">' + json.casts.crew[i].name, '</a><br />', json.casts.crew[i].job,'</div>');
                html.push('</a></li>');
                i = i+1;   
            }
            ...
            var i = 0;
            var html = [];
            var YTpath = "//www.youtube.com/watch?v=";
            while(i < total){
                html.push('<li>');
...
//                     alert ("Clicked inside " + href);
    ...
    var link = "//api.themoviedb.org/3/movie/" +  $(this).attr('href') + "/reviews";
...
//               console.dir(json);
            ...
            if (json.profile_path != null)                 // is there a picture
                {  
                $('#PeoplePoster').html('<img  class="FBimg" src="' + ImgPath + 'w154' + json.profile_path + '" title="' + json.name +'">' );
                }
            else
                {   
